...
def _convert_A(molecule_index, name, index,
    ...
    bonds = [0, 0, 0, 0, 0, 0]
    if name == 'A1':  # index 1, bonds 2
        atom_type = 'A1'
        atom_index = 1
        bonds[0] = index + 1
        total_bonds = 1
    elif name == 'A2':  # index 2, bonds 1
        atom_type = 'A2'
        atom_index = 2
        bonds[0] = index - 1
        total_bonds = 1
    else:
        raise ValueError(f'Unrecognized name in A: {name}')
