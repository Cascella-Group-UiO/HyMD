...
def test_comp_dihedrals(alanine_octapeptide):
    ...
    config.dihedrals = CONF['bond_4']
    _, _, bonds_4 = prepare_bonds(molecules, names, bonds, indices, config)
    # Calculated with 10 terms, don't remember why I used ten over five.
    # Probably need to recompute
    expected_energies = np.array([11.003619395008501,
                                  11.00061663675782,
                                  10.999290808649043,
                                  11.002169369896254,
                                  10.403862049596501], dtype=np.float64)
    expected_forces_i = np.array([
        [1.0033173067051937, 1.4360420258245659, 1.4510915277281982],
        [-1.2871093730639356, -1.1157993725821131, -2.627237367323105],
        [0.48977356759705426, 0.6274132149816739, 3.3400321539197364],
        [-0.14924708295812975, 0.41912527924936455, -2.6871875614263216],
        [4.491325225427978, 6.377163708487258, -17.68538890335337]],
        dtype=np.float64)
    expected_forces_j = np.array([
        [-1.0531585637443073, -1.9022449448729315, -1.1069671203423455],
        [1.9189016213804195, 1.5255101293127493, 2.324630891079988],
        [-0.7154097239880415, -1.494660164399034, -3.297218171490972],
        [0.7357059530429948, -0.05799732734744384, 2.825218623916153],
        [-3.3482368541148935, -3.415294328335266, 16.502210598504245]],
        dtype=np.float64)
    expected_forces_k = np.array([
        [-0.8837972846895308, -0.3431714207557409, -2.2498599382859936],
        [-0.18540797794264174, 0.16211959247864943, 3.3467434299124017],
        [0.03722332666608291, 1.3963600034712713, -3.4351791959715805],
        [-1.2214149485146613, -1.2626923358215993, 2.3622210056531614],
        [-6.464237394822091, -12.09241825555728, 18.232559680227162]],
        dtype=np.float64
    )
    expected_forces_l = np.array([
        [0.9336385417286444, 0.8093743398041066, 1.905735530900141],
        [-0.4463842703738423, -0.5718303492092855, -3.044136953669285],
        [0.18841282972490425, -0.5291130540539111, 3.392365213542816],
        [0.6349560784297963, 0.9015643839196785, -2.5002520681429927],
        [5.321149023509006, 9.130548875405289, -17.049381375378037]],
        dtype=np.float64
    )
