...
def make_whole(
    ...
    if out_path is None:
        out_path = (
            "".join(file_path.split(".")[:-1]) + "_whole." + file_path.split(".")[-1]
        )
    ...
    with h5py.File(out_path, "w") as out_file:
        ...
        box_sizes = out_file["/particles/all/box/edges/value"][...]
        ...
        out_file["/parameters/vmd_structure/bond_from"][...] = bond_from[
            bond_sort_ind
        ]  # noqa: E501
        out_file["/parameters/vmd_structure/bond_to"][...] = bond_to[
            bond_sort_ind
        ]  # noqa: E501
        ...
        if shift is not None:
            ...
            lenshape = out_file["/particles/all/position/value"][..., 0].shape[1]
            out_file["/particles/all/position/value"][..., 0] = np.mod(
                out_file["/particles/all/position/value"][..., 0],
                np.repeat(box_sizes[..., 0, 0][:, np.newaxis], lenshape, axis=1),
            )
            out_file["/particles/all/position/value"][..., 1] = np.mod(
                out_file["/particles/all/position/value"][..., 1],
                np.repeat(box_sizes[..., 1, 1][:, np.newaxis], lenshape, axis=1),
            )
            out_file["/particles/all/position/value"][..., 2] = np.mod(
                out_file["/particles/all/position/value"][..., 2],
                np.repeat(box_sizes[..., 2, 2][:, np.newaxis], lenshape, axis=1),
            )
        ...
        for frame in tqdm.tqdm(frames):
            ...
            for molecule_index in np.unique(molecules):
                ...
                for particle in particle_indices[0]:
                    ...
                    for dim in range(3):
                        if r[dim] > box_sizes[frame, dim, dim]:
                            positions[frame, particle, dim] -= box_sizes[
                                frame, dim, dim
                            ]
                        if r[dim] < 0.0:
                            positions[frame, particle, dim] += box_sizes[
                ...
                for particle in particle_indices[0]:
                    ...
                    for b in bonds:
                        for dim in range(3):
                            ...
                            dr = rj - ri
                            if dr > 0.5 * box_sizes[frame, dim, dim]:
                                positions[frame, b, dim] -= box_sizes[frame, dim, dim]
                            if dr <= -0.5 * box_sizes[frame, dim, dim]:
                                positions[frame, b, dim] += box_sizes[frame, dim, dim]
