...
def write_fort5(
    ...
    if n_polymers > 0:
        if polymer_topology is None and bonds is None:
            raise ValueError(
                "polymer_topology and bonds must be specified " "with n_polymers > 0"
            )
        elif polymer_topology is None:
            raise ValueError("polymer_topology must be specified with n_polymers > 0")
        elif bonds is None:
    ...
    if os.path.exists(out_path):
        if not force:
            raise FileExistsError(
                f"The file {out_path} already exists. Use "
    ...
    with open(out_path, "w") as out_file:
        ...
        for p in range(n_polymers):
            ...
            for a in range(n_atoms_per_polymer):
                ...
                out_file.write(
                    f"{atom_index:4}\t{atom_type_name:4}\t{atom_type:4}\t{num_bonds:4}"
                )  # noqa: E501
                out_file.write(
                    f"\t\t{position[0]:.16f}\t{position[1]:.16f}\t{position[2]:.16f}"
                )  # noqa: E501
        ...
        for s in range(n_solvent):
            ...
            out_file.write(
                f"{atom_index:4}\t{atom_type_name:4}\t{atom_type:4}\t{bonds:4}"
            )  # noqa: E501
...
if __name__ == "__main__":
    ...
    parser.add_argument("--box", type=int, nargs=3, help="simulation box length")
    parser.add_argument(
        "--n-molecules",
        type=int,
        dest="n_molecules",
    ...
    parser.add_argument(
        ...
        dest="out_path",
        metavar="file name",
        help="output fort.5 file name",
    )
    parser.add_argument(
        "-f", action="store_true", dest="force", help="overwrite existing output file"
    )
    ...
    write_fort5(
        ...
        args.n_molecules,
        args.n_solvent,
        polymer_topology=polymer_topology,
        bonds=bonds,
