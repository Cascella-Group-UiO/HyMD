...
mkdir ${SCRATCH}
cd ${SCRATCH}
cp ${SLURM_SUBMIT_DIR}/config.toml $SCRATCH/
cp ${SLURM_SUBMIT_DIR}/dppc_13x13x14.h5 $SCRATCH/
mkdir hymd/
cp /cluster/home/samiransen23/HyMD-2021/hymd/* hymd/

date
srun -K --error=${ERR_LOG} -n ${SLURM_NTASKS} --mpi=pmi2  python3 hymd/main.py config.toml dppc_13x13x14.h5 --logfile=${LOG_FILE}  --seed 1 --verbose 2
